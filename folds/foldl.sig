...
(* If `Either.INR c` is seen, short-circuit with `Either.INR c` *)
val left: ('s, 'a, 'b) t -> ('s, ('a, 'c) Either.either, ('b, 'c) Either.either) t
(* If `Either.INL c` is seen, short-circuit with `Either.INL c` *)
...
(* Run a fold, then apply the function on a fold initialized with the result of running the first fold *)
...
(* Combine inputs with a binary function. Returns NONE if there is no input *)
val reduce: ('a * 'a -> 'a) -> ('a option, 'a, 'a option) t
(* Convert to a list in the order inputs are seen *)
val list: ('a list -> 'a list, 'a, 'a list) t
(* Convert to a list, reversing the input *)
